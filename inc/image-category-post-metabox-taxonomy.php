		...
		?><script>
			jQuery(document).ready(function($){
				if ( typeof wp.media !== 'undefined' ) {
					...
					$('.categorys-media').click(function(e) {
						...
						_custom_media = true;
							...
							if ( _custom_media ) {
								...
								$('.image-view').attr( 'src', attachment.url );
							} else {
	...
	public function create_fields( $taxonomy ) {
		...
		foreach ( $this->meta_fields as $meta_field ) {
			...
			switch ( $meta_field['type'] ) {
				case 'media':
					$input = sprintf(
						'<img src="%s" class="image-view" ><input style="width: 80%%" id="%s" name="%s" type="text" value="%s"> <input style="width: 15%%" class="button categorys-media" id="%s_button" name="%s_button" type="button" value="Upload" />',
						$meta_value,
						$meta_field['id'],
						$meta_field['id'],
						$meta_value,
				...
				default:
					$input = sprintf(
						'<img src="%s" class="image-view"><input %s id="%s" name="%s" type="%s" value="%s">',
						$meta_value,
